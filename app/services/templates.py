            ...
            "schema": {
                ...
                "fields": {
                    "verdict": {"type": "single", "options": ["Correct", "Incorrect", "Partial"], "required": True,
                                "label": "Is the output clinically correct for this input?"},
                    "correct_label": {"type": "from_classes", "visible_when": "verdict!=Correct",
                                      "label": "What the answer should have been"},
                    "critical_miss": {"type": "structured",
                                      "label": "Which finding did it miss?",
                                      "finding_from_classes": True,
                                      "finding_label": "Which finding was missed"},
                    "notes": {"type": "text"},
                },
            },
        },
            ...
            "schema": {
                ...
                "fields": {
                    "verdict": {"type": "single", "required": True,
                                "label": "Is this output safe and correct for this patient?",
                                "options": ["Correct", "Incorrect", "Partial"]},
                    "severity": {"type": "single", "visible_when": "verdict!=Correct",
                                 "label": "How badly does it fail?",
                                 "options": ["Minor", "Moderate", "High", "Critical"],
                    ...
                    "red_flag_missed": {"type": "structured",
                                        "label": "Did the presentation contain a red flag the output ignored?",
                                        "finding_label": "Name the red flag",
                                        "hint": "A warning sign present in the case that the output did not act on"},
                    ...
                    "correct_label": {"type": "from_classes", "visible_when": "verdict!=Correct",
                                      "label": "What the answer should have been",
                                      "hint": "The class a correct output would have given"},
                    "critical_miss": {"type": "structured",
                                      "label": "Could this error harm the patient?",
                                      "finding_label": "What the harm would be",
                                      "hint": "Judge the consequence of the error itself, whatever caused it"},
                    "clinical_correctness": {"type": "scale", "max": 5,
                                             "label": "Clinical correctness (1 = wrong, 5 = fully correct)"},
                    "reasoning": {"type": "scale", "max": 5,
                                  "label": "Reasoning (1 = pattern-matched, 5 = reasoned)",
                                  "hint": "Did it reason from the presentation, or pattern-match?"},
                    "completeness": {"type": "scale", "max": 5,
                                     "label": "Completeness (1 = major omissions, 5 = complete)"},
                    "communication": {"type": "scale", "max": 5,
                                      "label": "Communication (1 = confusing, 5 = clear)"},
                    "rationale": {"type": "text", "rows": 4, "required": True,
                                  "label": "Why you judged it this way",
                                  "hint": "A clinician reading only this should follow your judgement"},
                },
            },
            "analytics": {
            ...
            "schema": {
                ...
                "fields": {
                    ...
                    "symptom_identification": {"type": "scale", "max": 5,
                        "label": "Symptom identification (1 = missed the important features, 5 = caught them)"},
                    "differential": {"type": "scale", "max": 5,
                        "label": "Differential (1 = missed the dangerous ones, 5 = considered them)"},
                    "follow_up_questions": {"type": "scale", "max": 5,
                        "label": "Follow-up questions (1 = asked none that matter, 5 = asked what a clinician would)"},
                    "recognised_missing_info": {"type": "scale", "max": 5,
                        "label": "Recognised what it did not know (1 = assumed, 5 = noticed the gaps)"},
                    "conclusion_justified": {"type": "scale", "max": 5,
                        "label": "Conclusion follows from what it established (1 = no, 5 = yes)"},
                    "error_category": {"type": "single", "visible_when": "verdict!=Correct",
            ...
            "schema": {
                ...
                "fields": {
                    ...
                    "citations_support_claims": {"type": "single",
                                                 "label": "Do the citations say what the answer claims they say?",
...
def config_from_template(name: str, classes: list[str] | None = None) -> dict | None:
    ...
    # The order the fields are authored in IS the order a clinician should be asked them:
    # verdict first, scales last. That order does not survive storage — eval_config is a
    # jsonb column and Postgres returns its keys sorted — so record it explicitly while we
    # still have it. build_label_config and the clinician platform both follow it.
    ec["schema"].setdefault("field_order", list((ec["schema"].get("fields") or {}).keys()))
    return ec
